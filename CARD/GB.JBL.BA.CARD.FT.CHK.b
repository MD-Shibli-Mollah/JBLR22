* @ValidationCode : MjoxODIzMDc0Mjg0OkNwMTI1MjoxNzA0MjYzODkxMDk3Om5hemloYXI6LTE6LTE6MDowOmZhbHNlOk4vQTpERVZfMjAxNzEwLjA6LTE6LTE=
* @ValidationInfo : Timestamp         : 03 Jan 2024 12:38:11
...
*-------init VAR--------------------------------------*
    ...
    Y.REQ.TYPE = EB.SystemTables.getRNew(EB.ATM19.REQUEST.TYPE)
...
* IF R.NEW(EB.ATM19.REQUEST.TYPE) EQ "REISSUE" AND R.NEW(EB.ATM19.REISSUE.REASON) EQ 5 THEN
        IF Y.REQ.TYPE EQ "REISSUE" AND Y.REISSUE.REASON EQ 5 THEN

        END
* ELSE IF R.NEW(EB.ATM19.REQUEST.TYPE) EQ "PINREISSUE" AND R.NEW(EB.ATM19.REISSUE.REASON) EQ 7 THEN
        ELSE IF Y.REQ.TYPE EQ "PINREISSUE" AND Y.REISSUE.REASON EQ 7 THEN

        END
    
        ELSE
* ETEXT ="PLEASE FIRST DEDUCT ":R.NEW(EB.ATM19.REQUEST.TYPE) :" FEE BY USING ATM Card Related Fee Deduction MENU THIS REQUEST"
            EB.SystemTables.setEtext("PLEASE FIRST DEDUCT ":Y.REQ.TYPE:" FEE BY USING ATM Card Related Fee Deduction MENU THIS REQUEST")
* EB.ErrorProcessing.StoreEndError()
            EB.ErrorProcessing.StoreEndError()
        END
    END
*    Y.REQUEST.TYPE=R.NEW(EB.ATM19.REQUEST.TYPE)
    Y.REQUEST.TYPE = Y.REQ.TYPE
...
* ETEXT="There must be a record ": Y.REQUEST.TYPE :" in JBL.ATM.MAINT.CALC"
        EB.SystemTables.setEtext("There must be a record ": Y.REQUEST.TYPE :" in JBL.ATM.MAINT.CALC")
        EB.ErrorProcessing.StoreEndError()
        RETURN
    END
...
* IF Y.TRN.TYPE NE REC.FT.CHK<FT.TRANSACTION.TYPE> THEN
            IF Y.TRN.TYPE NE Y.FT.CR.AC THEN
...
* IF  Y.VEN.AMT NE REC.FT.CHK<FT.DEBIT.AMOUNT> THEN
            IF  Y.VEN.AMT NE Y.FT.DR.AC THEN
